...
/datum/event/box/start()
	var/list/candidates
	var/mob/living/carbon/human/is_here
	var/obj/item/storage/box/funnybox
	...
	funnybox = pick(subtypesof(/obj/item/storage/box/event))

	new funnybox(get_turf(the_door))
	playsound(src, the_door.knock_sound, 50, 0, 3)
